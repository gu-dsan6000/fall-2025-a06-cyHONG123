...
def solve_problem1(spark):
    ...
    df = logs_df.select(regexp_extract('value', r'^(\d{2}/\d{2}/\d{2} \d{2}:\d{2}:\d{2})', 1).alias('ts_str')).withColumn('file_path', input_file_name())
        # i dropped all row without ts_str(time), since all task in prob2 require a time to be meaningful 
    df = df.filter(F.col("ts_str").isNotNull() & (F.col("ts_str") != ""))
    df = df.withColumn('timestamp',
        to_timestamp('ts_str', 'yy/MM/dd HH:mm:ss'))
